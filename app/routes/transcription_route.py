...
def process_audio():
    ...
    logger.info(f"Запрос транскрибации аудио с ID {audio_id}", extra={'user_id': current_user['login']})

    # Проверка на отсутствие audio_id или подсказки
    if not audio_id:
        logger.warning("Отсутствует audio_id.", extra={'user_id': current_user['login']})
        return jsonify({'msg': 'Missing audio ID.'}), 400
    ...
    # Выполняем полную обработку и транскрибацию аудио
    try:
        transcription_id = process_and_transcribe_audio(file_record, current_user['user_id'], audio_id)
